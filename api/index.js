...
async function handleMessage(event) {
  ...
  const resources = await searchBitableResources(text);
  if (resources.length === 0) {
    const hints = ["dealer pricing", "summer pallet", "monthly rebate", "warranty", "shipping policy", "new dealer", "POSM"].map(h => "  • " + h).join("\n");
  ...
  if (resources.length === 1) {
    ...
    let resultText = `📋 Found ${resources.length} resources:\n\n`;
    resources.forEach((r, i) => {
      resultText += `${i + 1}. ${r.title}`;
      if (r.resource_url) resultText += `\n   ${r.resource_url}`;
      resultText += "\n\n";
    });
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === "GET" && req.url?.includes("/health")) {
      return res.status(200).json({ status: "ok", bot: "Velotric Pricing Bot" });
    }
    if (req.method === "GET" && (req.url === "/" || req.url === "/api")) {
      return res.status(200).json({ name: "Velotric Pricing Bot", status: "running" });
    }
